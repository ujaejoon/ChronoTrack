...
class KITTIMem(BaseDataset):
    ...
    def __init__(self, split_type, cfg, log):
        ...
        if not cfg.debug:
            split_type_to_scene_ids = dict(train=list(range(0, 17)), val=list(range(17, 19)), test=list(range(19, 21)))
        else:
            split_type_to_scene_ids = dict(train=[0], val=[18], test=[19])
        ...
        if self.cache:
            if not cfg.debug:
                cache_file_dir = osp.join(
                    ...
                    f"KITTI_{self.cfg.category_name}_{split_type}_{self.cfg.coordinate_mode}_{self.preload_offset}_{cfg.num_smp_frames_per_tracklet}frames_{cfg.frame_npts}pts_maskscale{cfg.mask_scale}.cache",
                )
            else:
                cache_file_dir = osp.join(
                    self.cfg.data_root_dir,
                    f"KITTI_DEBUG_{self.cfg.category_name}_{split_type}_{self.cfg.coordinate_mode}_{self.preload_offset}_{cfg.num_smp_frames_per_tracklet}frames_{cfg.frame_npts}pts_maskscale{cfg.mask_scale}.cache",
                )
            if osp.exists(cache_file_dir):
                ...
                for tracklet_id in tqdm(
                    ...
                    if self.preload_offset > 0:
                        for frame in frames:
                            frame["pcd"] = crop_pcd_axis_aligned(frame["pcd"], frame["bbox"], offset=self.preload_offset)

                    tracklets.append({"comp_template_pcd": comp_template_pcd, "frames": frames})

                with open(cache_file_dir, "wb") as f:
                    self.log.info(f"Saving data to cache file {cache_file_dir}")
        ...
        if split_type == "train":
            self.tracklet_annotations, self.tracklets = self.filter_tracklet(self.tracklet_annotations, self.tracklets, cfg.num_smp_frames_per_tracklet)

        self.tracklet_num_frames = [len(tracklet_anno) for tracklet_anno in self.tracklet_annotations]
    ...
    def get_tracklet_frame_id(self, idx):
        tracklet_id = bisect.bisect_right(self.tracklet_ed_frame_id, idx)
        assert self.tracklet_st_frame_id[tracklet_id] <= idx and idx < self.tracklet_ed_frame_id[tracklet_id]
        frame_id = idx - self.tracklet_st_frame_id[tracklet_id]
    ...
    def _build_tracklet_annotations(self, scene_ids):
        ...
        for scene_id in tqdm(scene_ids, desc="[%6s]Loading annos" % self.split_type.upper(), disable=pl_ddp_rank() != 0):
            annotation_file_dir = osp.join(self.cfg.data_root_dir, "label_02/%04d.txt" % scene_id)
            data = pd.read_csv(
                annotation_file_dir,
                sep=" ",
                names=[
            ...
            for track_id in track_ids:
                ...
                tracklet_anno = [frame_anno for _, frame_anno in tracklet_anno.iterrows()]
    ...
    def _build_frame(self, frame_anno, bbox_only=False):
        ...
        if scene_id in self.calibration_info:
            ...
            calib = self._read_calibration_file(osp.join(self.cfg.data_root_dir, "calib/%04d.txt" % scene_id))
        ...
        if self.cfg.coordinate_mode == "camera":
            bbox_center = [frame_anno["x"], frame_anno["y"] - frame_anno["height"] / 2, frame_anno["z"]]
            size = [frame_anno["width"], frame_anno["length"], frame_anno["height"]]
            orientation = Quaternion(axis=[0, 1, 0], radians=frame_anno["rotation_y"]) * Quaternion(axis=[1, 0, 0], radians=np.pi / 2)
            bbox = BoundingBox(bbox_center, size, orientation)
        else:
            box_center_cam = np.array([frame_anno["x"], frame_anno["y"] - frame_anno["height"] / 2, frame_anno["z"], 1])
            ...
            size = [frame_anno["width"], frame_anno["length"], frame_anno["height"]]
            orientation = Quaternion(axis=[0, 0, -1], radians=frame_anno["rotation_y"]) * Quaternion(axis=[0, 0, -1], degrees=90)
            bbox = BoundingBox(box_center_velo, size, orientation)
        if bbox_only:
            return {"bbox": bbox, "anno": frame_anno}
        try:
            pcd_file_dir = osp.join(self.cfg.data_root_dir, "velodyne/%04d" % scene_id, "%06d.bin" % frame_id)
            pcd = PointCloud(np.fromfile(pcd_file_dir, dtype=np.float32).reshape(-1, 4).T)
            if self.cfg.coordinate_mode == "camera":
...
class TrainDatasetWrapper(torch.utils.data.Dataset):
        ...
        for i, (bbox, pcd, prev_bbox) in enumerate(zip(frame_bboxes, frame_pcds, prev_frame_bboxes)):
            ...
            if i == 0:
                ...
                base_bbox = get_offset_box(
                    prev_bbox,
                    bbox_offset,
                    use_z=self.cfg.train_cfg.use_z,
                    offset_max=self.cfg.offset_max,
            ...
            pcd = crop_and_center_pcd(
                pcd,
                base_bbox,
                offset=self.cfg.frame_offset,
                offset2=self.cfg.frame_offset2,
                scale=self.cfg.frame_scale,
            )
            mask_gt = get_pcd_in_box_mask(pcd, bbox, scale=self.cfg.mask_scale).astype(int)
            bbox_gt = np.array(
                [
                    bbox.center[0],
                    bbox.center[1],
            ...
            pcd, idx = resample_pcd(pcd, self.cfg.frame_npts, return_idx=True, is_training=True)
            mask_gt = mask_gt[idx]
    ...
    def _to_float_tensor(self, data):
        ...
        for k, v in data.items():
            # -------------------------------------------------
            # if v is list convert to numpy array
    ...
    def __getitem__(self, idx):
        ...
        frame_ids = [frame_id]
        acceptable_set = set(range(max(0, frame_id - self.cfg.max_frame_dis), min(tracklet_length, frame_id + self.cfg.max_frame_dis + 1))).difference(set(frame_ids))
        while len(frame_ids) < self.cfg.num_smp_frames_per_tracklet:
            idx = np.random.choice(list(acceptable_set))
            frame_ids.append(idx)
            new_set = set(range(max(0, idx - self.cfg.max_frame_dis), min(tracklet_length, idx + self.cfg.max_frame_dis + 1)))
            acceptable_set = acceptable_set.union(new_set).difference(set(frame_ids))

        frame_ids = sorted(frame_ids)
        if np.random.rand() < 0.5:
            # Reverse time
            frame_ids = frame_ids[::-1]
            prev_frame_ids = [min(f_id + 1, tracklet_length - 1) for f_id in frame_ids]
        else:
            prev_frame_ids = [max(f_id - 1, 0) for f_id in frame_ids]

        frames = [self.dataset.get_frame(tracklet_id, id) for id in frame_ids]
        prev_frame_bboxes = [self.dataset.get_frame_bbox(tracklet_id, id) for id in prev_frame_ids]
