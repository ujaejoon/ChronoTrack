...
class Waymo(BaseDataset):
    ...
    def __init__(self, split_type, cfg, log):
        ...
        bench_dir = osp.join(cfg.data_root_dir, "benchmark", "validation", cfg.category_name.lower())
        ...
        self.easy_ids = extract_ids_from_bench("easy.json") #374
        self.medium_ids = extract_ids_from_bench("medium.json") # 374
        self.hard_ids = extract_ids_from_bench("hard.json") # 373
    ...
    def get_frame(self, tracklet_id, frame_id):
        ...
        if tracklet_id != self.cache_tracklet_id:
            ...
            if self.pcds:
                del self.pcds
            self.gt_infos = np.load(osp.join(self.cfg.data_root_dir, "gt_info", "{:}.npz".format(segment_name)), allow_pickle=True)
            self.pcds = np.load(osp.join(self.cfg.data_root_dir, "pc", "raw_pc", "{:}.npz".format(segment_name)), allow_pickle=True)
    ...
    def _build_frame(self, frame_range, t_id, frame_id):
        ...
        pcd = PointCloud(np.asarray(self.pcds[str(idx)].T, dtype=np.float32))
