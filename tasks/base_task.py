...
class BaseTask(L.LightningModule):
    ...
    def __init__(self, cfg, log):
        ...
        if "KITTI" in cfg.dataset_cfg.dataset_type:
            ...
            raise NotImplementedError("%s has not been supported!" % cfg.dataset_cfg.dataset_type)
    ...
    def training_step(self, *args, **kwargs):
        raise NotImplementedError("Training_step has not been implemented!")
    ...
    def forward_on_tracklet(self, tracklet):
        raise NotImplementedError("Forward_on_tracklet has not been implemented!")
    ...
    def validation_step(self, batch, batch_idx):
        ...
        for pred_bbox, gt_bbox in zip(pred_bboxes, gt_bboxes):
            overlap = estimateOverlap(gt_bbox, pred_bbox, dim=self.cfg.eval_cfg.iou_space, up_axis=self.cfg.dataset_cfg.up_axis)
            accuracy = estimateAccuracy(gt_bbox, pred_bbox, dim=self.cfg.eval_cfg.iou_space, up_axis=self.cfg.dataset_cfg.up_axis)
            overlaps.append(overlap)
            accuracies.append(accuracy)

        self.succ(torch.tensor(overlaps, device=self.device))
        self.prec(torch.tensor(accuracies, device=self.device))
        self.runtime(torch.tensor(runtime, device=self.device), torch.tensor(n_frames, device=self.device))
    ...
    def _on_test_epoch_start_kitti_format(self):
        ...
        self.runtime.reset()
        self.n_frames.reset()
        if self.cfg.model_cfg.model_type == "ChronoTrack":
            with torch.no_grad():
                for _ in range(100):
                    backbone_input = dict(
                    ...
                    trfm_input = dict(
                        ...
                        memory=dict(
                            feat=torch.randn(1, 128, 1, 128).cuda(),
                            xyz=torch.randn(1, 1, 128, 3).cuda(),
                            mask=torch.randn(1, 1, 128).cuda(),
                            fg_tokens=torch.randn(1, 32, 128).cuda(),
                        ),
                    )
                    loc_input = dict(
                        xyz=torch.randn(1, 128, 3).cuda(),
                    ...
                    update_input = dict(
                        xyz=torch.randn(1, 128, 3).cuda(),
                        geo_feats=torch.randn(1, 128, 128).cuda(),
                        mask=torch.ones(1, 128).cuda(),
                        memory=dict(
                            feat=torch.randn(1, 128, 1, 128).cuda(),
                            xyz=torch.randn(1, 1, 128, 3).cuda(),
                            mask=torch.ones(1, 1, 128).cuda(),
                            fg_tokens=torch.randn(1, 32, 128).cuda(),
                        ),
                    )
                    ...
                    _ = self.model(update_input, "update")
    ...
    def _test_step_kitti_format(self, batch, batch_idx):
        ...
        self.runtime(torch.tensor(runtime, device=self.device), torch.tensor(n_frames, device=self.device))
        self.n_frames(torch.tensor(n_frames, device=self.device))
        self.txt_log.info("Prec=%.3f Succ=%.3f Frames=%d RunTime=%.6f" % (self.prec.compute(), self.succ.compute(), self.n_frames.compute(), self.runtime.compute()))
        self.log("precesion", self.prec.compute(), prog_bar=True, logger=False, batch_size=1)
        self.log("success", self.succ.compute(), prog_bar=True, logger=False, batch_size=1)
        self.log("n_frames", self.n_frames.compute(), prog_bar=True, logger=False, batch_size=1)
    ...
    def _test_step_waymo_format(self, batch, batch_idx):
        ...
        overlaps, accuracies = [], []
        for pred_bbox, gt_bbox in zip(pred_bboxes, gt_bboxes):
            overlap = estimateWaymoOverlap(gt_bbox, pred_bbox, dim=self.cfg.eval_cfg.iou_space)
            accuracy = estimateAccuracy(gt_bbox, pred_bbox, dim=self.cfg.eval_cfg.iou_space, up_axis=self.cfg.dataset_cfg.up_axis)
            overlaps.append(overlap)
            accuracies.append(accuracy)
        success(torch.tensor(overlaps, device=self.device))
        precision(torch.tensor(accuracies, device=self.device))
        success = success.compute() if type(success.compute()) == float else success.compute().item()
        precision = precision.compute() if type(precision.compute()) == float else precision.compute().item()
        ...
        self.txt_log.info("Total: Prec=%.3f Succ=%.3f Frames=%d" % (self.prec_total.avg, self.succ_total.avg, self.n_frames_total))
        self.txt_log.info("easy: Prec=%.3f Succ=%.3f Frames=%d" % (self.prec_easy.avg, self.succ_easy.avg, self.n_frames_easy))
        self.txt_log.info("medium: Prec=%.3f Succ=%.3f Frames=%d" % (self.prec_medium.avg, self.succ_medium.avg, self.n_frames_medium))
        self.txt_log.info("hard: Prec=%.3f Succ=%.3f Frames=%d" % (self.prec_hard.avg, self.succ_hard.avg, self.n_frames_hard))
    ...
    def _test_step_nuscenes_format(self, batch, batch_idx):
        ...
        self.succ(torch.tensor(overlaps, device=self.device))
        self.prec(torch.tensor(accuracies, device=self.device))
        self.txt_log.info("Key: Prec=%.3f Succ=%.3f Key Frames=(%d/%d)" % (self.prec.compute(), self.succ.compute(), self.n_frames_key, self.n_frames_total))
    ...
    def _on_test_epoch_end_kitti_format(self):
        ...
        self.log("success", self.succ.compute(), prog_bar=True)
        self.log("runtime", self.runtime.compute(), prog_bar=True)
        self.txt_log.info("Avg Prec/Succ=%.3f/%.3f Frames=%d Runtime=%.6f" % (self.prec.compute(), self.succ.compute(), self.n_frames.compute(), self.runtime.compute()))
        self.txt_log.info(f'Max allocated memory (MB): {torch.cuda.max_memory_allocated() / (2**20)}')
        if self.cfg.save_test_result:
