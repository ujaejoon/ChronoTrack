...
class EdgeConv(nn.Module):
    ...
    def get_graph_feature(self, new_xyz, new_feat, xyz, feat, k, use_xyz=False):
        ...
        # b,n,c
        _, knn_idx, _ = pytorch3d.ops.knn_points(new_feat, feat, K=k, return_nn=True)
    ...
    def forward(self, xyz, feat, npoints):
        ...
        """
        device = xyz.device
        _, sample_idx = pytorch3d.ops.sample_farthest_points(xyz, K=npoints)
        sample_idx = sample_idx.long().to(device)

        new_xyz = torch.gather(xyz, 1, sample_idx.unsqueeze(-1).expand(-1, -1, 3))
        new_feat = torch.gather(feat, 2, sample_idx.unsqueeze(1).expand(-1, feat.size(1), -1)) if feat is not None else None
        ...
        for i in range(len(l_idxs)):
            for bi in range(bs):
                if i == 0:
                    ...
                    idxs_list[i].append(idxs_list[i - 1][bi][l_idxs[i][bi].long()])

        idx = torch.stack(idxs_list[len(l_idxs) - 1], dim=0)

        return dict(xyz=l_xyz[-1], feat=self.fc(l_features[-1]), idx=idx.long(), geo_feat=l_features[-1])
